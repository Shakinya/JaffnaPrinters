...
export default function PageHero({ badge, title, description, minHeight = 'lg' }: PageHeroProps) {
  return (
    <section
      className={`page-hero relative ${heights[minHeight]} flex items-end bg-slate-50 overflow-hidden`}
    >
      ...
      <div className="container-custom relative z-10 pb-8 sm:pb-10">
        <motion.div variants={staggerContainer} initial="hidden" animate="show">
          <motion.span
            ...
            className="inline-flex items-center gap-2 px-4 py-1.5 rounded-btn bg-brand-red/10 text-brand-red text-xs font-semibold font-display tracking-widest uppercase mb-4"
          >
            {badge}
          </motion.span>
          <motion.h1
            ...
            className="font-display font-bold text-brand-charcoal text-3xl sm:text-4xl lg:text-5xl leading-tight mb-3"
          >
            {title}
          </motion.h1>
          <motion.p
            ...
            className="text-slate-600 text-base sm:text-lg max-w-2xl"
          >
            {description}
          </motion.p>
        </motion.div>
